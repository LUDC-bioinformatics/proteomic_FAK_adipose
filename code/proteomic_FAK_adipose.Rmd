...
```{r}
colnames(data)[c(11:22)] <- gsub(' ', '_', gsub(', ', '_', colnames(data)[c(11:22)]))
labels <- colnames(data)[c(11:22)]
...
# Generate a SummarizedExperiment object
data_se <- make_se(data, c(11:22), expDesign)
data_se
```
...
plot_single(dep,
            proteins = c('CAH4', 'KT33A', 'ACAD9', 'ARPC4', 'DCMC', 'ARPC2'),
            type = "contrast")
```
...
plot_single(dep,
            proteins = c('CAH4', 'KT33A', 'ACAD9', 'ARPC4', 'DCMC', 'CAZA2'),
            type = "centered")
```
...
save(data_diff, data_imp, data_norm, data_se, dep, file = 'intermediate/proteomic_FAK_adipose.RData')
...
```{r}
write.table(df_res,
            file = 'results/table/proteomic_FAK_adipose_results.tsv',
            quote = T,
            row.names = F,
            sep = '\t')
```
