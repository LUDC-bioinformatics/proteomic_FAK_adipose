...
plot_volcano_publication <- function(dep, contrast, label_size = 3, add_names = TRUE, adjusted = FALSE, 
    ...
    if (add_names) {
        p <- p + ggrepel::geom_text_repel(data = filter(df, name %in% lable_genes), 
...
Genes to label in plots:

```{r}
genes_to_remove <- c("Gnb1", "Calm2", "Gnai1", "Ca4", "Prxl2a", "Mlycd")

FAK_PF_INS_vs_FAK_DMSO_INS_sign_names <- data_results %>% filter(FAK_PF_INS_vs_FAK_DMSO_INS_significant) %>% pull(name)
FAK_PF_INS_vs_FAK_DMSO_INS_sign_names <- FAK_PF_INS_vs_FAK_DMSO_INS_sign_names[!FAK_PF_INS_vs_FAK_DMSO_INS_sign_names %in% genes_to_remove]
FAK_PF_INS_vs_FAK_DMSO_INS_sign_names
```
...
fig3B <- plot_volcano_publication(dep, contrast = "FAK_DMSO_INS_vs_FAK_DMSO_basal",
                                  ...
                                  lable_genes = c('Ca4', 'Gnai1', 'Gnai2', 'Gnb1', 'Rpl36', 'Purb'))
fig3B

# pdf('results/figures/fig3B_FAK_DMSO_INS_vs_FAK_DMSO_basal.pdf',  height = 5, width = 5)
#   print(fig3B)
# dev.off()
```
...
# pdf('results/figures/fig3C_FAK_PF_INS_vs_FAK_DMSO_INS.pdf',  height = 5, width = 5)
#   print(fig3C)
# dev.off()
```
